# ruff: noqa: E501
"""Parse CSV/XLSX endpoint - AI-powered batch transaction extraction."""
...
<identity>
You are a financial transaction parser. Convert tabular CSV/Excel rows into structured JSON transactions.
</identity>
...
3. Accept any column naming style and infer field meaning from headers and row content.
...
async def parse_batch(
    ...
    """Parse CSV/XLSX rows into structured transactions."""
    ...
    if (
        extension not in CSV_EXTENSIONS | EXCEL_EXTENSIONS
        and content_type not in ALLOWED_CONTENT_TYPES
    ):
        return JSONResponse(
            status_code=400, content={"error": "Only CSV and XLSX files are supported"}
        )
